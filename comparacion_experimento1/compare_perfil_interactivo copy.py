...
class PerfilViewBox(pg.ViewBox):
    ...
    def _to_view(self, scene_pos):
        """Convierte posición de escena a coordenadas de vista (mm) sin mapSceneToView."""
        sr = self.sceneBoundingRect()
        vr = self.viewRect()
        if sr.width() == 0 or sr.height() == 0:
            return None
        fx = (scene_pos.x() - sr.left()) / sr.width()
        fy = (scene_pos.y() - sr.top())  / sr.height()
        x = vr.x() + fx * vr.width()
        # escena Y crece hacia abajo; vista Y crece hacia arriba → invertir
        y = vr.y() + (1.0 - fy) * vr.height()
        return x, y
    ...
    def mouseDragEvent(self, event, axis=None):
        if event.button() != QtCore.Qt.MouseButton.LeftButton:
            ...
            return
        if event.isStart():
            c = self._to_view(event.buttonDownScenePos())
            if c is not None:
                self.clicked.emit(c[0], c[1])
        event.accept()
